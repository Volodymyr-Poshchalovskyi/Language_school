...
function LearningFormPage({ data }) {
  return (
    ...
    <div
      className={`${data.bgColor} dark:bg-gray-900 py-16 md:py-24 px-4 sm:px-6 lg:px-8`}
    >
      <div className="max-w-4xl mx-auto">
        ...
        <div className="avoid-emoji bg-white dark:bg-gray-800 rounded-2xl shadow-xl dark:shadow-none p-8 md:p-12">
          <div className="mb-10">
            ...
            <ul className="space-y-3 text-lg text-gray-700 dark:text-gray-300">
              {data.forWhomList.map((item, index) => (
                <li key={index} className="flex items-start">
                  <span
                    className={`${data.forWhomCheckmarkColor} mr-3 mt-1 dark:opacity-90`}
                  >
                    &#10003;
                  </span>
                  <span>{item}</span>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {data.advantages.map((advantage, index) => (
                <div
                  key={index}
                  className={`${advantage.bgColor} dark:bg-gray-700 p-6 rounded-lg`}
                >
                  <h3
                    className={`font-bold text-xl ${advantage.titleColor} dark:text-gray-100 mb-2`}
                  >
                    {advantage.title}
                  </h3>
                  <p
                    className={`${advantage.descriptionColor} dark:text-gray-200`}
                  >
        ...
        <div className="avoid-emoji mt-12 text-center">
          <Link
            to={data.buttonLink}
            className={`inline-block ${data.buttonColor} text-white font-bold py-3 px-8 rounded-lg text-lg ${data.buttonHoverColor} transition-transform transform hover:scale-105 dark:shadow-none`}
          >
            {data.buttonText}
          </Link>
        </div>
